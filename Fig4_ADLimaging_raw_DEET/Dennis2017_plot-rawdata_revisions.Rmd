...
datafromfilelist <- read.csv((paste(wdplus,file_list[i],sep="")),header=FALSE)

frame <- c(1:4680)
animal <- c(1:dim(datafromfilelist)[1])
video <- rep(i,dim(datafromfilelist)[1])

arbitraryunits <- datafromfilelist-(min(datafromfilelist))
...
animal <- rep(i,1500)
datafromfilelist <- read.csv((paste(wdplus,file_list[i],sep="")),header=FALSE)[,2]
arbitraryunits <- datafromfilelist-(min(datafromfilelist))
data <- as.data.frame(cbind(animal,frame,arbitraryunits))
...
animal <- rep(i,1500)
datafromfilelist <- read.csv((paste(wdplus,file_list[i],sep="")),header=FALSE)
datafromfilelist <- datafromfilelist[,2]
arbitraryunits <- datafromfilelist-(min(datafromfilelist))
data <- as.data.frame(cbind(animal,frame,arbitraryunits))
finaldf <- data
